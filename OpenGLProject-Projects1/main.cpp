...
bool showDepthMap;
float rotationAngle = 0.0f; // Unghiul de rotație al morișcii
...
void keyboardCallback(GLFWwindow* window, int key, int scancode, int action, int mode) {
	...
	if (key == GLFW_KEY_R && action == GLFW_PRESS)
		isRotating = !isRotating;  // Schimbă starea rotației
...
{
	static float lastTime = glfwGetTime(); // Reține timpul precedent
	float currentTime = glfwGetTime();    // Obține timpul curent
	float deltaTime = currentTime - lastTime; // Timp scurs între cadre
	lastTime = currentTime;

	// Increment continuu pentru unghi
	angleY += 50.0f * deltaTime;
	angleD += 50.0f * deltaTime;// 
...
void initObjects() {
	...
	moara.LoadModel("objects/mill/mill.obj");
	morisca.LoadModel("objects/morisca/morisca.obj");
	nisip.LoadModel("objects/nisip/nisip.obj");
	sat.LoadModel("objects/sat/sat.obj");
}
...
void initUniforms() {
	...
	//set light color
	lightColor = glm::vec3(1.0f, 0.9f, 0.8f); //white light
...
void drawObjects(gps::Shader shader, bool depthPass) {
	...
	// Transformări pentru mcqueen
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(13.0f, 0.19f, 0.0f));
	model = glm::scale(model, glm::vec3(0.005f)); // Scalare redusă
	model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(glGetUniformLocation(shader.shaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(model));

	// Nu trimitem matricea normală în cazul unui depth pass
	if (!depthPass) {
		normalMatrix = glm::mat3(glm::inverseTranspose(view * model));
		glUniformMatrix3fv(normalMatrixLoc, 1, GL_FALSE, glm::value_ptr(normalMatrix));
	}
	...
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(36.0f, 0.35f, -3.0f));
	model = glm::scale(model, glm::vec3(1.3f)); // Scalare redusă
	...
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(35.0f, 3.6f, 0.0f));
	model = glm::scale(model, glm::vec3(0.8f)); // Scalare redusă
	...
	model = glm::translate(glm::mat4(1.0f), glm::vec3(30.0f, -2.25f, 0.0f));
	model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
